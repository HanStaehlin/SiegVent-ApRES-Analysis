...
from velocity_profile import (load_phase_data, calculate_velocity_profile,
                              save_velocity_results, estimate_noise_floor)
...
# min_snr_db=3 to include weaker/deeper layers (speckle tracking for low SNR)
layers = detect_layers(data['range_img'], data['Rcoarse'], min_depth=50, max_depth=1050, min_snr_db=3, min_persistence=0.5)
...
# Step 3: Velocity Profile with Kingslake uncertainty
print('\n' + '='*60)
print('STEP 3: VELOCITY PROFILE (with Kingslake uncertainty)')
print('='*60)
phase_data = load_phase_data(str(output_path / 'phase_tracking'))
noise_floor = estimate_noise_floor(data['range_img'])
print(f'  Noise floor: {noise_floor:.4f} ({20*np.log10(noise_floor):.1f} dB)')
velocity_result = calculate_velocity_profile(
    phase_data,
    r_sq_threshold=0.3,
    amp_threshold_db=-80,
    noise_floor=noise_floor,
    raw_image=data['range_img'],
)
...
if n_reliable > 0:
    ...
    if velocity_result.uncertainty_kingslake is not None:
        print(f'  Kingslake σ: median {np.median(velocity_result.uncertainty_kingslake[reliable_mask]):.4f} m/yr')
    if velocity_result.uncertainty_wls is not None:
        print(f'  WLS σ: median {np.median(velocity_result.uncertainty_wls[reliable_mask]):.4f} m/yr')
